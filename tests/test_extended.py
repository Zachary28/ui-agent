...
def test_plan_writes_manifest_and_result(tmp_path):
    result = run(
        AutomationRequest(
            platform="browser", target={"url": "http://example.test"}, goal="inspect", report_dir=str(tmp_path)
        )
    )
...
def test_vitest_marker_create_and_update(tmp_path):
    adapter = VitestE2EAdapter()
    path = adapter.create(str(tmp_path), "login", "verify login")
    assert "aiAct" in path.read_text()
    adapter.update_case(str(tmp_path), "login", "verify success")
    assert "verify success" in path.read_text()
...
def test_discover_native_report_is_run_scoped(tmp_path):
    report = tmp_path / "midscene_run" / "report" / "one"
