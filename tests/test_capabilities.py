...
def test_all_platforms_build_required_core_commands():
    ...
    targets = {
        ...
        "computer": {},
        "android": {"device_id": "a"},
        "ios": {"wda_host": "127.0.0.1", "wda_port": 8100},
        "harmony": {"device_id": "h"},
    }
    for platform, target in targets.items():
        ...
        for op in ("connect", "health_check", "screenshot", "assert", "tap_locate", "report", "disconnect"):
            ...
            req = AutomationRequest(
                ...
                target=target,
                goal="x",
                operation="tap_locate" if op == "tap_locate" else op,
                locate=payload,
            )
...
def test_launch_uri_is_not_forwarded_to_connect():
    req = AutomationRequest(
        platform="android", target={"device_id": "a", "app_uri": "tv.danmaku.bili"}, goal="launch", operation="connect"
    )
