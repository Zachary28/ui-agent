...
def test_loop_runtime_runs_ticks_until_runtime_limit(tmp_path):
    ...
    plan = LoopPlan.model_validate(
        {
            "defaults": {"interval_seconds": 1},
            "exit_conditions": {"max_runtime_seconds": 0.01},
            "operations": {"check_playback": {"enabled": True, "interval_seconds": 1}},
        }
    )
...
def test_loop_runtime_cancel_is_clean(tmp_path):
    ...
    result = workflow.run(
        LoopPlan.model_validate(
            {"exit_conditions": {"max_runtime_seconds": 1}, "operations": {"screenshot": {"enabled": True}}}
        ),
    ...
    request = AutomationRequest.model_validate(
        {
            ...
            "loop": {
                "exit_conditions": {"max_runtime_seconds": 0.01},
                "operations": {"check_playback": {"enabled": True}},
            },
        }
    )
