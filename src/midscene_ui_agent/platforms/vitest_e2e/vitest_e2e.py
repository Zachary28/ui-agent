...
class VitestE2EAdapter(PlatformAdapter):
    ...
    def init(self, project_dir: str, platform: str, *, ai_action_context: str = "") -> list[Path]:
        if platform not in {"web", "android", "ios"}:
            raise UiAgentError(ErrorCode.VALIDATION_ERROR, f"invalid Vitest platform: {platform}")
    ...
    def create(self, project_dir: str, name: str, prompt: str, *, update: bool = False) -> Path:
        ...
        test_dir = project / "e2e"
        test_dir.mkdir(exist_ok=True)
        path = test_dir / f"{name}.test.ts"
        if path.exists() and not update:
            raise UiAgentError(ErrorCode.COMMAND_FAILED, f"test already exists: {path}")
        content = self.render_case(name, prompt)
    ...
    def update_case(self, project_dir: str, name: str, prompt: str) -> Path:
        project = Path(project_dir)
        matches = list(project.rglob("*.test.ts"))
        for path in matches:
            text = path.read_text(encoding="utf-8")
            start = f"// ui-agent:case {name}:start"
            end = f"// ui-agent:case {name}:end"
            if start in text and end in text:
                a, b = text.index(start), text.index(end) + len(end)
                replacement = self.render_case(name, prompt).rstrip()
                tmp = path.with_suffix(path.suffix + ".tmp")
                tmp.write_text(text[:a] + replacement + text[b:], encoding="utf-8")
                tmp.replace(path)
                return path
    ...
    def command(self, request: AutomationRequest, operation: str | None = None) -> CommandSpec:
        ...
        project = Path(request.target.project_dir or ".")
        if op == "run":
            argv = ["pnpm", "vitest", "run"]
        elif op == "debug":
            argv = ["pnpm", "vitest", "run", "-t", request.test_name or ""]
        elif op in {"init", "convert", "create", "update"}:
            argv = ["ui-agent-vitest", op, "--platform", request.target.vitest_platform or ""]
        else:
            raise UiAgentError(ErrorCode.UNSUPPORTED_OPERATION, op)
        return CommandSpec(
            argv,
            cwd=str(project),
            timeout_seconds=request.timeout_seconds,
            session_id=f"vitest:{project.resolve()}:{request.target.vitest_platform}",
        )
