...
app=typer.Typer()
@app.command()
def run(platform:str=typer.Option(...,"--platform"), goal:str=typer.Option(...,"--goal"), url:str|None=None, device_id:str|None=None, operation:str="run", mode:str="plan", report_dir:str="./artifacts", project_dir:str|None=None, vitest_platform:str|None=None, app_uri:str|None=typer.Option(None,"--launch-uri"), cdp:str|None=None, bridge:bool=False, wda_host:str|None=None, wda_port:int|None=None, raw_command:str|None=None, raw_method:str|None=None, raw_endpoint:str|None=None, locate_json:str|None=None, deep_think:bool=False, deep_locate:bool=False, resume_id:str|None=typer.Option(None,"--resume"), run_id:str|None=None, image:list[str]=typer.Option([],"--image"), image_name:list[str]=typer.Option([],"--image-name"), convert_http_image2_base64:bool=typer.Option(False,"--convert-http-image2-base64"), check_dependencies:bool=typer.Option(False,"--check-dependencies"), json_output:bool=typer.Option(False,"--json")):
    ...
    result=execute(AutomationRequest(platform=platform,target=target,goal=goal,operation=operation,mode=mode,report_dir=report_dir,run_id=run_id or resume_id,raw_command=raw_command,raw_method=raw_method,raw_endpoint=raw_endpoint,locate=locate,deep_think=deep_think,deep_locate=deep_locate),resume=bool(resume_id)); typer.echo(result.model_dump_json() if json_output else f"{result.status}: {result.run_id}")
@app.command("vitest")
