...
def prepare_run(state: AutomationGraphState) -> dict[str, str]:
    return {"phase": "prepare_run", "status": state.get("status") or "running"}


def verify_skill_lock(state: AutomationGraphState) -> dict[str, str]:
    return {"phase": "verify_skill_lock", "status": state.get("status") or "running"}


def execute_route(state: AutomationGraphState) -> dict[str, str]:
    return {"phase": "execute_route", "status": state.get("status") or "running"}
...
def finalize_run(state: AutomationGraphState) -> dict[str, str]:
    ...
    return {
        "phase": "finalize_run",
        "status": "succeeded" if status in {None, "running"} else str(status),
    }
