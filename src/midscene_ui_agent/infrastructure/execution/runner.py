...
class CommandSpec:
    ...
    sensitive_indexes: set[int] = field(default_factory=set)
...
class CommandRunner:
    ...
    def run(self, spec: CommandSpec, *, run_id: str = "adhoc", event_id: str = "command") -> CommandResult:
        ...
        with lock:
            ...
            try:
                ...
                raise UiAgentError(ErrorCode.DEPENDENCY_NOT_FOUND, str(exc)) from exc
            try:
            ...
            if timed_out:
                raise UiAgentError(
                    ErrorCode.TIMEOUT,
                    "command timed out: " + " ".join(summarize_argv(spec.argv, spec.sensitive_indexes)),
                    retryable=True,
                )
    ...
    def _write_logs(self, run_id: str, event_id: str, stdout: str, stderr: str) -> None:
        for stream, text in (("stdout", stdout), ("stderr", stderr)):
            path = self.artifact_root / run_id / stream / f"{event_id}.log"
