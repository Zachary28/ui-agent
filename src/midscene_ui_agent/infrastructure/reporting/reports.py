...
from ...domain.contracts import AutomationResult
...
def build_manifest(
    result: AutomationResult, request, root: str | Path, *, thread_id=None, fingerprints=None, graph_phase=None
) -> Path:
    root = Path(root)
...
def convert_native_report(
    package: str, html_path: str | Path, output_dir: str | Path, runner: CommandRunner, *, run_id="report"
) -> list[Path]:
        ...
        result = runner.run(
            CommandSpec(
                [
                    ...
                    html_path,
                    "--outputDir",
                    str(output_dir / suffix),
                ],
                session_id=f"report:{html_path}",
            ),
            run_id=run_id,
            event_id=f"report-{suffix}",
        )
        if result.returncode:
            raise RuntimeError(result.stderr or f"report {action} failed")
